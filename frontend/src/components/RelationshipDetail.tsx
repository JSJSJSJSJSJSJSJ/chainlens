...
import { dateLabel, reviewLabels, statusLabels, temporalLabels, validityLabels } from '../labels';
  ...
  return <div className="drawer-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <aside className="detail-drawer" role="dialog" aria-modal="true" aria-labelledby="detail-title" tabIndex={-1} ref={panel}>
      ...
      {relation.loading ? <Loading text="正在读取关系详情…" /> : relation.error ? <Failure message={relation.error} onRetry={() => setRevision(value => value + 1)} /> : relation.data && <div className="drawer-content">
        <div className="detail-intro"><Direction relation={relation.data} /><h3>{relation.data.business_description}</h3><div className="detail-badges"><span className={`status-label status-${relation.data.fact_status}`}>{statusLabels[relation.data.fact_status]}</span><span className="review-tag">{reviewLabels[relation.data.human_review_status]}</span><span className="soft-tag">{temporalLabels[relation.data.temporal_status]}</span></div></div>
