...
- relationship: id, source_company_id, target_company_id, relationship_type (`supplier`|`partner`|`investor_or_investee`|`peer`), business_description, fact_status (`confirmed`|`inferred`|`unknown`), valid_from (date|null), valid_to (date|null), temporal_status (`historical`|`current`|`unknown`), continuity_note, uncertainty, human_review_status (`pending`|`review_prepared`|`approved`，分别表示待审核、草稿已准备、人工审核通过), entity_resolution (`exact`|`mapped`|`uncertain`), comparison_dimension (string|null), quantitative_context (string|null), evidence_links ([{evidence_id, stance: `supports`|`contradicts`, directness: `direct`|`indirect`}]).
