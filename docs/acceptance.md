...
当前默认数据集为`2026-09-16.v3`，22条关系均为`approved`，审核人为JOE、日期为2026-09-22。本地默认数据库已导入v3。v1/v2及manifest保留。

本次实际运行：41项后端测试、7项组件测试、2项真实后端浏览器测试全部通过；Ruff、TypeScript、Vite构建、manifest摘要校验和`git diff --check`通过。浏览器验证了v3版本及详情中的“人工审核通过”。逐项比较v2/v3确认公司、证据及关系事实未变，仅审核状态更新；评分不因审核状态改变的回归测试通过。第三方弃用提示仍存在。两条Playwright用例均输出ok，但命令在服务退出阶段未正常结束，已尝试中断；不将该命令记为正常退出。

下方保留2026-09-16的原始验收结果，其中pending和旧测试数量仅描述当时状态。

## 十项要求映射（2026-09-16历史记录）
...
1. 关系人工审核已于2026-09-22由JOE完成，见[逐条记录](human-review.md)；历史ASUS采购与投资的时间边界保留。整体方法观点仍为AI草稿。
